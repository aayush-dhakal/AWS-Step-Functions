const AWS = require("aws-sdk"); // this is available in lambda context, we don't have to do anything
...
const deductPoints = async (userId) => {
  let params = {
    ...
    ExpressionAttributeValues: {
      ":zero": 0, // since orderAmount is grater than points so we set it to zero
    },
  };
...
module.exports.checkInventory = async ({ bookId, quantity }) => {
  try {
    ...
    if (isBookAvailable(book, quantity)) {
      ...
      bookOutOfStockError.name = "BookOutOfStock"; // we are referencing this error name from state machine error
...
module.exports.calculateTotal = async ({ book, quantity }) => {
  // book is passed as result from previous state, and quantity is the input that was provided in the first state of step function. We have access to both as defined via ResultPath in checkInventory state
  console.log("book: ", book);
  let total = book.price * quantity;
  return { total }; // returns as { total: total}
};
...
module.exports.restoreRedeemPoints = async ({ userId, total }) => {
  try {
    // we ge the total from previous state returned values(ie from calculateTotal state)
    if (total.points) {
      let params = {
        TableName: "userTable",
        Key: { userId: userId },
